...
const normalizeHtml=text=>strip(text).replace(/(<meta name="oc-build" content=")[a-f0-9]{12}(">)/,'$1$2');
const id=createHash('sha256').update(JSON.stringify(contents)+css+normalizeHtml(original)).digest('hex').slice(0,12);
let changed=false;
...
const html=normalizeHtml(original).replace('<meta name="oc-build" content="">',`<meta name="oc-build" content="${id}">`).replace('src="src/app.js"',`src="src/app.js?build=${id}"`).replace('href="app.css"',`href="app.css?build=${id}"`);
