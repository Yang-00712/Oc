...
try{
 ...
 const result=await page.evaluate(async forms=>{
  const {segment}=await import('./src/segmentation.js'),{prepareLine,decodeCTC,recognitionBox}=await import('./src/paddle.js'),{loadEngineAssets}=await import('./src/engine-assets.js');
  ...
  try{
   for(const form of forms){
    ...
    for(const method of ['current','padded','cell','contrast','adopted']){
     ...
     for(let i=0;i<pieces.length;i++){
      ...
      if(method==='adopted')box=recognitionBox(piece,{rowMode:'grid30'});
 ...
 for(const report of reports){const baseline=report.methods.find(m=>m.method==='current'),adopted=report.methods.find(m=>m.method==='adopted'),padded=report.methods.find(m=>m.method==='padded');assert.deepEqual(adopted.rows.map(r=>r.raw),padded.rows.map(r=>r.raw));assert.ok(adopted.exact>=baseline.exact,report.scenario+' exact matches regressed');assert.ok(adopted.characterErrors<=baseline.characterErrors,report.scenario+' character errors regressed');}
