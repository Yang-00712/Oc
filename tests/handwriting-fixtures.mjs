...
export const scenarios=['clear','shadow','tight'];
const strokes={
...
export function makeForm(scenario='clear'){
 ...
 const tilted=scenario==='tight',offset=y=>tilted?-15*(y-110)/(lines.at(-1)-110):0;
 const elements=[],bottom=lines.at(-1),color=scenario==='shadow'?'url(#paper)':'#ebe8de';
 ...
 targets.forEach((value,row)=>{
  ...
  elements.push('<text x="30" y="'+(y+h*.65)+'" fill="#6b6d63" font-size="11">R'+String(row+1).padStart(2,'0')+' / note</text>');
  [...value].forEach((digit,col)=>{
   const x=286+offset(y)+col*(tilted?21.5:24)+(random()-.5)*2,dy=y+(h-23)/2+(random()-.5)*2,angle=(random()-.5)*(tilted?12:6);
   const strokeWidth=(tilted?1.3:1.5)+(random()-.5)*.3;
   const opacity=scenario==='shadow'&&[1,4,8,12,18,25].includes(row)&&col===3?.42:.88;
   elements.push('<g transform="translate('+x.toFixed(2)+' '+dy.toFixed(2)+') rotate('+angle.toFixed(2)+' 10 13) scale(1 .83)" stroke="#344b84" stroke-opacity="'+opacity+'" stroke-width="'+strokeWidth.toFixed(2)+'" stroke-linecap="round" stroke-linejoin="round" fill="none">'+strokes[digit].map(d=>'<path d="'+d+'"/>').join('')+'</g>');
  });
 });
