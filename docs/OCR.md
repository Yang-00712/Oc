...
`gridColumn` 先用原 RGBA 的暗色中性格線像素（亮度 <155，色差 <=40 或最大色值 <=70），在中央84%高度分12帶追蹤細縱框；每帶覆蓋至少72%且連續55%，至少9帶支持才線性擬合。兩側依實際y移動，避免微傾斜縱框在全高固定x投影消失。格線搜尋改以欄寬計算72%覆蓋／55%連續；保留原有線數、間距、缺外框與黑帶驗證。相鄰縱框間淨寬至少為裁切高度/30（且>=8）；較窄邊欄排除，多個可用資料欄拒絕。每格依兩側線的內緣裁切模型輸入與縮圖，色塊和旁欄不混入空白格。無足夠縱框時仍走原嚴格全寬偵測，不降低門檻硬湊格位。

格線深色判定另需局部對比：使用半徑 9–25 px 的平均亮度，像素須比附近紙面暗至少 14 且亮度低於 155；亮度低於 70 的真黑墨保留，避免把大片黑色遮蓋拆成虛假的細線。這可排除底部陰影造成的假橫線，合成測試同時保留黑帶拒絕規則。


## 合成手寫壓力測試與辨識留白

`tests/handwriting.html` 可預覽／下載四種 30 格 PNG：一般藍筆、陰影與淡筆畫、密字距與微傾斜、不規則筆形。字形以自訂向量筆畫繪製，seed 固定 712；沒有使用使用者照片、私人表單、教材或外部字型。固定答案在 `tests/handwriting-fixtures.mjs`，包含重複與非遞增時間，不能依時間排序猜補。

`tools/handwriting-benchmark.mjs` 以同一個 PP-OCRv5 通用模型、同一組分格結果，比較原墨跡裁圖、額外留白、完整格內範圍、灰階對比及正式採用的 `recognitionBox`。Actions 的 `test-report/handwriting.json` 保存逐列答案、原始辨識、整列正確數、字元編輯錯誤、漏字與測試執行時間；PNG 一起放在 artifact。這些是同一組 30 列的四種合成變化，不是 120 個獨立真人樣本，也不代表手機耗時或使用者筆跡準確率。

`recognitionBox` 只對 grid30 在原墨跡裁圖四周增加 max(3px, 原框高度 × 22%) 的留白，限制在同一格內且避開已偵測格線；手動指定的缺失外框如不適用，就保留原框。一次模型推論，不增加並行、不訓練、不改數字解碼、不補字。原始轉錄與人工確認流程保留。採用方式須通過四種情境的整列正確數不下降及字元錯誤不增加檢查。


本次驗證（Actions run 35501947067，模型權重不變）：

| 合成情境 | 原墨跡裁圖完全正確 | 採用留白後完全正確 |
|---|---:|---:|
| 一般藍筆 | 30/30 | 30/30 |
| 陰影與淡筆畫 | 29/30 | 30/30 |
| 密字距與微傾斜 | 30/30 | 30/30 |
...
較難筆形的完整格內裁圖及灰階對比各為 29/30，故未採用。這次每格仍只推論一次；合成測試不是實拍準確率或更換模型的證據。
