// This cache contains public model files only. Never enumerate/delete site storage.
...
export async function manifest(){
 const r=await fetch(new URL('models/local-engines.json',base),{cache:'no-cache'});
...
export async function verifiedAsset(asset,notify=()=>{}){
...
export async function loadEngineAssets(id,notify){
 const m=await manifest(),entry=m.models[id];if(!entry||entry.files.length!==2)throw new Error('模型不在已驗證清單內。');
 const config=JSON.parse(new TextDecoder().decode(await verifiedAsset(entry.files.find(f=>f.path.endsWith('config.json')),notify)));
 ...
 const weights=await verifiedAsset(item,notify);
