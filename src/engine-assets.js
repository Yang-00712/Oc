// Window owns model storage; inference workers only receive transferred bytes.
import { readModelFile, writeModelFile, modelFileKeys, removeModelFiles } from './model-cache.js?build=80613cecd6f2';
...
export async function verifiedAsset(asset,notify=()=>{},signal){
 checkCancelled(signal);
 const url=assetUrl(asset);let storageAvailable=true,cached;
 try{cached=await readModelFile(url);}catch{storageAvailable=false;notify('此瀏覽器無法保存模型；本次仍可辨識。');}
 checkCancelled(signal);
 if(cached!==undefined){const bytes=cached;if(bytes instanceof ArrayBuffer&&bytes.byteLength===asset.bytes&&await sha(bytes)===asset.sha256)return bytes;throw new Error('已下載模型完整性不符；請只移除此模型快取後重新下載，草稿不受影響。');}
 ...
 checkCancelled(signal);
 if(storageAvailable)try{await writeModelFile(url,bytes);}catch{notify('模型已下載，但無法保存；本次仍可辨識，草稿獨立保存。');}
 checkCancelled(signal);
 return bytes;
}
...
export async function modelStorage(){
 const m=await manifest(),keys=new Set(await modelFileKeys()),items=[];
 for(const [id,entry] of Object.entries(m.models)){
  let saved=0;for(const file of entry.files){if(keys.has(assetUrl(file)))saved+=file.bytes;}
...
export async function clearModel(id){
 ...
 // Only this engine's files (including previous hashes). No database deletion.
