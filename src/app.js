import { parseTime, exportTimes, orderWarnings } from './time.js?build=cfe03be0eec5';
import { decodePhoto, rotatedPhoto, cropPhoto, thumbnail } from './photo.js?build=cfe03be0eec5';
import { engineById, selectedEngines, comparisonStats } from './engines.js?build=cfe03be0eec5';
import { restoreDraft, snapshotDraft } from './multi-state.js?build=cfe03be0eec5';
import { startUpdates } from './update.js?build=cfe03be0eec5';
import * as store from './store.js?build=cfe03be0eec5';

const $=id=>document.getElementById(id);
const SCAN_ENGINE='ppocr-v5-ch';
let actionCount=0,draftSaveError=null,interacted=false;
for(const event of ['pointerdown','keydown','input'])document.addEventListener(event,()=>{interacted=true;},{capture:true,passive:true});
...
function protect(action){return async event=>{actionCount++;try{await action(event);}catch(error){notice(error.message||String(error),true);}finally{actionCount--;}};}
function tab(name){
...
function save(){
    ...
    saving=saving.catch(()=>{}).then(()=>store.setState('draft',snapshot)).then(()=>{draftSaveError=null;$('save-status').textContent='已保存本機草稿（不含整張照片）。';}).catch(error=>{
...
function refreshRow(row,element,index){
    ...
    if(row.raw)messages.push(`${row.numericOnly?'數字初判':'初判'} ${row.raw}`);
    if(row.numericOnly&&typeof row.transcript==='string'&&row.transcript!==row.raw)messages.push(`限定前原始辨識：${row.transcript||'（空白）'}`);
...
function updateRowMode(){
    ...
    $('row-count-field').hidden=grid;$('grid-edges-field').hidden=!grid;
    $('row-mode-help').textContent=grid
        ? ($('grid-edges').value==='auto'?'請框住完整 30 格與上下格線。只缺外框時可在「表單外框」指定框選邊緣；旁邊文字留在框外。':'請把框選上下緣對齊完整 30 格的外緣。指定缺線端會使用框選邊界，第一／最後格需確認數字沒有切掉；中間仍依實際格線分格。')
        : '自由手寫時，列數只用來核對，不會等高硬切；請框近時間欄。';
}
$('row-mode').onchange=updateRowMode;$('grid-edges').onchange=updateRowMode;updateRowMode();
    ...
    return new Promise((resolve,reject)=>{
        ...
        void (async()=>{
            ...
            if(engine.kind==='line'){
                ...
                const {loadEngineAssets}=await import('./engine-assets.js?build=cfe03be0eec5');
                if(id!==job||settled)return;
                const {withAssetBudget}=await import('./asset-download.js?build=cfe03be0eec5');
            ...
            worker=new Worker(new URL(engine.id==='cnn'?'./ocr-worker.js?build=cfe03be0eec5':'./line-worker.js?build=cfe03be0eec5',import.meta.url),{type:'module'});
...
$('recognize').onclick=protect(async()=>{
    ...
    const options={rowMode:$('row-mode').value,gridEdges:$('grid-edges').value,rowCount:$('row-mode').value==='grid30'?'30':$('row-count').value,digitMode:$('digit-mode').value};
    const completed=[];let hasSuccess=false;
    batchRunning=true;queueCancelled=false;$('recognize').disabled=true;$('row-mode').disabled=true;$('grid-edges').disabled=true;$('cancel').hidden=false;
    try{
        ...
        endWorker();batchRunning=false;crop.width=1;crop.height=1;$('recognize').disabled=!photo;$('row-mode').disabled=false;$('grid-edges').disabled=false;$('cancel').hidden=true;
...
async function showModelStorage(){
    const {modelStorage,clearModel}=await import('./engine-assets.js?build=cfe03be0eec5'),info=await modelStorage(),host=$('model-storage');host.replaceChildren();
...
async function prepareModels(ids){
    ...
    await runInstallation(async(report,signal)=>{
        const {prepareEngine}=await import('./engine-assets.js?build=cfe03be0eec5');
...
$('model-package').onchange=protect(async event=>{
    ...
    await runInstallation(async(report,signal)=>{const {importModelPack}=await import('./model-pack.js?build=cfe03be0eec5');await importModelPack(file,report,signal);});
});
...
$('export-training').onclick=protect(async()=>{
    ...
    const {zipFiles}=await import('./zip.js?build=cfe03be0eec5');
    const labels=[],files=[];
...
startUpdates({
    canAutoReload:()=>!interacted&&!photo&&!batchRunning&&!installing&&!actionCount&&storageSafe,
    blockReload:()=>batchRunning||installing||actionCount?'請等目前操作完成，或先取消，再更新。':'',
    prepareReload:async()=>{
        if(!storageSafe)throw new Error('本機儲存不可用，請先匯出；已停止更新。');
        // Input and asynchronous saves may have queued more than one transaction.
        let pending;do{pending=saving;await pending;}while(pending!==saving);
        if(draftSaveError)throw new Error('草稿保存失敗，請先匯出；已停止更新。');
    }
});
