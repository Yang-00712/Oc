import { parseTime, exportTimes, orderWarnings } from './time.js?build=aea212fe8d9d';
import { decodePhoto, rotatedPhoto, cropPhoto, thumbnail } from './photo.js?build=aea212fe8d9d';
import { engineById, selectedEngines, comparisonStats } from './engines.js?build=aea212fe8d9d';
import { restoreDraft, snapshotDraft } from './multi-state.js?build=aea212fe8d9d';
import { startUpdates } from './update.js?build=aea212fe8d9d';
import * as store from './store.js?build=aea212fe8d9d';
    ...
    return new Promise((resolve,reject)=>{
        ...
        void (async()=>{
            ...
            if(engine.kind==='line'){
                ...
                const {loadEngineAssets}=await import('./engine-assets.js?build=aea212fe8d9d');
                if(id!==job||settled)return;
                const {withAssetBudget}=await import('./asset-download.js?build=aea212fe8d9d');
            ...
            worker=new Worker(new URL(engine.id==='cnn'?'./ocr-worker.js?build=aea212fe8d9d':'./line-worker.js?build=aea212fe8d9d',import.meta.url),{type:'module'});
...
async function showModelStorage(){
    const {modelStorage,clearModel}=await import('./engine-assets.js?build=aea212fe8d9d'),info=await modelStorage(),host=$('model-storage');host.replaceChildren();
...
async function prepareModels(ids){
    ...
    await runInstallation(async(report,signal)=>{
        const {prepareEngine}=await import('./engine-assets.js?build=aea212fe8d9d');
...
$('model-package').onchange=protect(async event=>{
    ...
    await runInstallation(async(report,signal)=>{const {importModelPack}=await import('./model-pack.js?build=aea212fe8d9d');await importModelPack(file,report,signal);});
});
...
$('export-training').onclick=protect(async()=>{
    ...
    const {zipFiles}=await import('./zip.js?build=aea212fe8d9d');
    const labels=[],files=[];
